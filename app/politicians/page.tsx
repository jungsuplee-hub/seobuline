...
export default function PoliticiansPage() {
  ...
  const filtered = useMemo(
    () =>
      visibleItems.filter((item) => {
        ...
        const byQuery = !query || [item.name, item.summary, item.stance_or_relevance, item.district].join(" ").toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-md border-l-4 border-[#d0a453] bg-[#121721] p-3 text-sm">공개 출처 기반 정보 아카이브: 공식 홈페이지·선관위·언론·의정활동 등 공개 확인 자료만 반영합니다.</div>
      <div className="grid gap-2 md:grid-cols-4">
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="이름/지역/요약 검색" />
      </div>
      <p className="text-sm text-[#cab898]">검색 결과: {filtered.length}명</p>
      {filtered.map((item) => (
        <Card key={`${item.name}-${item.office_type}-${item.district}`}>
          ...
          <div className="mt-2 flex flex-wrap gap-3 text-xs">
            {item.official_website && <a className="underline" href={item.official_website} target="_blank" rel="noreferrer">공식 홈페이지</a>}
