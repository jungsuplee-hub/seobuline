...
export default async function ResourcesPage() {
  const [user, resources] = await Promise.all([
    getCurrentUser(),
    Promise.resolve(db.prepare("SELECT * FROM resources ORDER BY created_at DESC").all() as Array<{ id: number; title: string; url: string; category: string | null; thumbnail_url: string | null; created_at: string }>),
  ]);
    ...
    <div className="space-y-4">
      ...
      {resources.map((resource) => (
        <Card key={resource.id}>
          ...
          <p className="text-sm text-slate-500">{resource.category || "기타"} · {resource.created_at}</p>
          <a className="mt-2 inline-block text-sm text-primary underline" href={resource.url} target="_blank" rel="noreferrer">자료 보기</a>
